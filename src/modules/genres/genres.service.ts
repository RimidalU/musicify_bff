...
export class GenreService extends RESTDataSource {
	...
	async genre(id: string): Promise<IGenre> {
		const data = await this.get(`/${id}`);
		return data;
	}
	async genres(limit = 5, offset = 0): Promise<IGenres> {
		const data = await this.get('', { limit: limit, offset: offset });
		return data.items;
	}
	async createGenre(data: IGenre): Promise<IGenre> {
	...
	async updateGenre(data: IGenre): Promise<IGenre> {
